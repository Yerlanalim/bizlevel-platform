...
export default function Profile() {
    ...
    const getUser = async () => {
      try {
        ...
        if (user) {
          ...
          // Если пользователь не авторизован, создаем демо-профиль
          setProfile({
            id: 'demo',
            ...
            skills: [
              {
                ...
                level: 3,
                maxLevel: 10,
                tooltips: Array(10).fill('Демо-подсказка')
              },
              {
                name: 'Управление и планирование',
                level: 2,
            ...
            achievements: [
              { 
                title: 'Демо достижение', 
                icon: '🔍', 
                description: 'Ознакомление с платформой'
              }
            ],
            contacts: {
              phone: '',
  ...
  // Если профиль не загрузился, показываем запасной вариант
  if (!profile) {
    return (
      <div className="max-w-4xl mx-auto p-6 font-sans">
        <div className="bg-yellow-50 p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-bold text-yellow-800 mb-2">Профиль недоступен</h2>
          <p className="text-yellow-700 mb-4">
            Не удалось загрузить данные профиля. Пожалуйста, попробуйте позже или обратитесь в поддержку.
          </p>
        </div>
      </div>
    )
